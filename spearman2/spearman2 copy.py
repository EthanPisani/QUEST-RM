...
#         comparison_data.append({'Model': model, 'rpbench_name': rpbench_name})
...
ax1.scatter(rpbench_ranks, your_ranks)
for i, model in enumerate(your_ranking):
    ax1.text(rpbench_ranks[i]+0.1, your_ranks[i]+0.1, model, fontsize=8)
ax1.plot([1, 12], [1, 12], 'k--', alpha=0.3)
ax1.set_xlabel("RPBench Rank")
ax1.set_ylabel("Your Model Rank")
ax1.set_title(f"Ranking Comparison\nSpearman ρ = {spearman_corr:.2f}, Kendall τ = {kendall_corr:.2f}")
ax1.grid(True)
...
ax2.set_xticks([])
ax2.set_title("Rank Differences Heatmap (Positive = Your Ranking is Higher)")
...
sns.barplot(data=corr_df.sort_values('Correlation', ascending=False), x='Attribute', y='Correlation', palette='viridis', ax=ax4)
...
plt.tight_layout()
plt.savefig(f'comprehensive_ranking_analysis_{args.rankings_csv.split("/")[-1].split(".")[0]}.png', dpi=300)
plt.show()
